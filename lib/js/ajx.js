              ...
              success: (res) => {
                if (isJson(res)) {
                  ...
                  chunks.forEach((item, index) => {
                    if (item.order_name == chnkOID) {
                      targetRow.setAttribute('data-downloaded', item.downloaded);

                      let finalSize = (item.byte_end - item.byte_start);
                      let finalProg = item.downloaded * 100;
                          finalProg = finalProg / finalSize;
                          finalProg = parseInt(finalProg);
                      if (item.downloaded == 0) finalProg = 0;

                      targetRow.children[2].innerText = finalProg+'%';
                      targetRow.children[3].innerText = chnkUID;
                    }
                  });
                }
              },
              complete: () => { console.log('[!] Fetch chunk request completed'); },
              load: 'up'
            });
          })();
        }
...
function loadJDS(jdsArray) {
  ...
  if (jdsArray.hasOwnProperty('membership')) {
    // if (jdsArray['membership'] == 'new_member') {
    if (true) {
      // [LOAD THE NEW JOINT GROUP] ----------------------------->
      ajx({ type: 'POST', url: '/JDS/req/req_handler.php', data: {uData: true},
        success: (res) => {
                                          ...
                                          if (isJson(res)) {
                                            ...
                                            chunks.forEach((item, index) => {
                                              ...
                                              let sR2_row = document.createElement('tr');
                                                  ...
                                                  sR2_row.setAttribute('data-downloaded', item.downloaded);
                                                  ...
                                                  for (var i = 0; i < 5; i++) {
                                                    let sR2_row_field_0 = document.createElement('td');
                                                        ...
                                                        if (i == 2) sR2_row_field_0.innerText = finalProg+'%';
