...
function ajx(obj) {
  ...
  let timeOut = (!obj.hasOwnProperty('timeOut')) ? ()=>{Swal.fire({ icon: "error", title: "Server timedout", text: "There seems to be a problem with the internet connection." });} : obj.timeOut;
  ...
  ajaxProcess[JSON.stringify(loaderID)] = $.ajax({
    ...
    success: function(result) {
      if (isJson(result)) {
        ...
        if (res.session_id == "") {
          Swal.fire({icon: "error",title: "error",text: "Session expired, please log in again"});
          return false;
        }
      }
      success(result);
    ...
    error: function(requestObject, error, errorThrown) {
      ...
      if (err != undefined) {
        Swal.fire({icon: "error",title: "Unexpected error",text: "AJX+SERVER: "+err});
      } else {
            ...
            for (var j = 0; j < download.length; j++) {
              let mainRow = document.createElement('DIV');
                  ...
                  let subRow_1 = document.createElement('DIV');
                      ...
                      let subRow_1_1 = document.createElement('DIV');
                          ...
                          let subRow_1_1_1 = document.createElement('DIV');
                              ...
                              let subRow_1_1_1_1 = document.createElement('A');
                                  subRow_1_1_1_1.setAttribute('class', 'ext');  // sub row 1 1 1 1
                                  subRow_1_1_1_1.innerText = "EXT"; // file extension
                          ...
                          let subRow_1_2_1 = document.createElement('DIV');
                              ...
                              for (var div = 0; div < 4; div++) {  // sub row 1 2 1 <->  sub row 1 2 1 (1<->4) 3
                                let subRow_1_2_1_0 = document.createElement('DIV');
                                    ...
                                    switch (div) {
                                      case 0: // URL
                                        subRow_1_2_1_0_1.innerText = "https://www.domain.com/example/music.mp3";
                                        subRow_1_2_1_0_3.innerText = "URL"; // name of field
                                        break;

                                      case 1: // SIZE
                                        subRow_1_2_1_0_1.innerText = "---";
                                        subRow_1_2_1_0_3.innerText = "Estimated size"; // name of field
                                        break;

                                      case 2: // STATUS
                                        subRow_1_2_1_0_1.innerText = "Waiting";
                      ...
                      let subRow_1_3 = document.createElement('DIV');
                          subRow_1_3.setAttribute('class', '_jdsiri_row'); // child
