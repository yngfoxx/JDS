...
function connectsocket(channel, uid) {
  ...
  jdschannel.on('msg', (data) => {
    ...
    } else if (data.hasOwnProperty('rtDownloadProg')) {
      let chnkID = data.chunkCID;
      let chnkJID = data.chunkJID;
      let chnkRID = data.chunkRID;
      let chnkOID = data.chunkOID;
      let chnkPROG = data.chunkPROGRESS;
      let chnkSIZE = data.chunkSIZE;
      let chnkUID = data.userID;
      let chnkB_Start = data.B_START;
      let chnkB_End = data.B_END;

      try {
        let targetRow = document.querySelector('._jdsi_row_opt_row[data-rt-id=\"'+chnkJID+'_'+chnkRID+'_'+chnkOID+'\"]');
        if (targetRow) {
          // targetRow.children[0]; // Order number
          // targetRow.children[1]; // Range
          // targetRow.children[2]; // Progress
          // targetRow.children[3]; // Users
          let realChunkSIZE = targetRow.getAttribute('data-size');
          let realChunkB_START = targetRow.getAttribute('data-B-start');
          let realChunkB_END = targetRow.getAttribute('data-B-end');

          if (!targetRow.hasAttribute('data-downloaded')) {
            targetRow.setAttribute('data-downloaded', chnkSIZE);
          }

          let rChnkDloadedSize = targetRow.getAttribute('data-downloaded');
              rChnkDloadedSize += chnkSIZE;

          let finalProgress = rChnkDloadedSize * 100
              finalProgress = finalProgress / realChunkB_END
              finalProgress = finalProgress + '%';

          /*
            100 = realChunkB_END
            x = rChnkDloadedSize
            x * realChunkB_END = rChnkDloadedSize * 100
            x = (rChnkDloadedSize * 100) / realChunkB_END
          */
          targetRow.children[2].innerText = finalProgress;
          targetRow.children[3].innerText = chnkUID;
        }
      } catch (e) {
                                ...
                                switch (div1) {
                                  ...
                                  case 1:
                                    ...
                                    let subRow_1_2_2_0_4 = document.createElement('A');
                                        ...
                                        subRow_1_2_2_0.setAttribute('data-jid', FILE_JID);
                      ...
                      let ov_container = document.createElement('DIV');
                          ...
                          let ov_content = document.createElement('DIV');
                              ...
                              let ov_table = document.createElement('TABLE');
                                        ...
                                        if (x == 0) {
                                          for (var i = 0; i < 4; i++) {
                                            let sR2_head_field_0 = document.createElement('th');
                                                ...
                                                if (i == 1) sR2_head_field_0.innerText = "Range";
                                                if (i == 2) sR2_head_field_0.innerText = "Progress";
                                                if (i == 3) sR2_head_field_0.innerText = "Users";
                                  ...
                                  // Fetch chunk data
                                  let chnkJID = FILE_MANAGER_BUTTON.getAttribute('data-jid');
                                  let chnkRID = FILE_MANAGER_BUTTON.getAttribute('data-rid');
                                  if (chnkJID != '' && chnkRID != '') {
                                    (() => {
                                      ajx({
                                        type: 'POST',
                                        url: '/JDS/req/req_handler.php',
                                        data: {jdsChunks: true, jid: chnkJID, rid: chnkRID},
                                        success: (res) => {
                                          if (isJson(res)) {
                                            let chunks = JSON.parse(res);
                                            chunks.forEach((item, index) => {
                                              // Chunk ----------------------------------------->
                                              let sR2_row = document.createElement('tr');
                                                  sR2_row.setAttribute('class', '_jdsi_row_opt_row');
                                                  sR2_row.setAttribute('data-rt-id', chnkJID+'_'+chnkRID+'_'+item.order_name);
                                                  sR2_row.setAttribute('data-size', (item.byte_end - item.byte_start));
                                                  sR2_row.setAttribute('data-B-start', item.byte_start);
                                                  sR2_row.setAttribute('data-B-end', item.byte_end);
                                                  for (var i = 0; i < 5; i++) {
                                                    let sR2_row_field_0 = document.createElement('td');
                                                        sR2_row_field_0.setAttribute('class', '_jdsiroh_field');
                                                        // if (i == 0) sR2_row_field_0.innerText = index;
                                                        if (i == 0) sR2_row_field_0.innerText = item.order_name;
                                                        if (i == 1) sR2_row_field_0.innerText = item.byte_start+" - "+item.byte_end;
                                                        if (i == 2) sR2_row_field_0.innerText = "N/A";
                                                        if (i == 3) sR2_row_field_0.innerText = "N/A";
                                        ...
                                        complete: () => {
                                          console.log('[!] Fetch chunk request completed');
                                        },
                                        load: 'up'
                                      });
                                    })();
                                  ...
                                  //       let sR2_row_field_0 = document.createElement('td');
                                  //           sR2_row_field_0.setAttribute('class', '_jdsiroh_field');
                                  //           if (i == 0) sR2_row_field_0.innerText = "N/A";
                                  //           if (i == 1) sR2_row_field_0.innerText = "N/A";
                                  //           if (i == 2) sR2_row_field_0.innerText = "N/A - N/A";
                                  //           if (i == 3) sR2_row_field_0.innerText = "N/A";
                                  //           if (i == 4) sR2_row_field_0.innerText = "N/A";
                                  //       sR2_row.append(sR2_row_field_0);
                                  //     }
                                  // ov_table.append(sR2_row);
                                  // -------------------------------------------------->
