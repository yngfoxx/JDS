      ...
      if (res != 0) {
        if (isJson(res)) {
          ...
          if (jRes.hasOwnProperty('server_error')) {
            ...
            const jdsChannel = io('https://ws-jds-eu.herokuapp.com/py_'+GROUP_CHANNEL, {
              reconnectionDelayMax: 10000,
              query: { auth: 'qPyFMKAdjtfL3Gq5pk2xDgy0SKMpEmLz', uid: jRes.UID },
            });

            jdsChannel.on('connect', () => {
              console.log('{SOCKET} => ['+jdsChannel.id+'] NOW LISTENING TO '+GROUP_CHANNEL);
              // OBJ: update socket id in database
              // request memebers status in socket channel
              jdsChannel.emit('msg', {foo: 'bar'});
              console.log("sent hello message");
            });

            // ----------------------------- [SOCKET MESSAGE HANDLER FOR SOCKET GROUP] -------------------------------- \\
            jdsChannel.on('msg', (data) => {
              // OBJ: Process incoming messages
              ...
              if (data.hasOwnProperty('response')) {
                ...
                switch (data.response) {
                  case 'user_status':
                    ...
                    let userDiv = document.querySelector('._stat[data-uid="'+uData.uid+'"]');
                    userDiv.innerHTML = (uData.status == 'connected') ? "<font color='#4bc64b'>"+uData.status+"</font>" : ((uData.status == 'disconnected') ? "<font color='grey'>"+uData.status+"</font>" : "---");
                    break;
                  default:
            ...
            for (var i = 0; i < member.length; i++) {
              ...
              jdsChannel.emit('msg', {action: 'user_status', targetUID: member[i].uid});
            }
