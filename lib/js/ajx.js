        ...
        if (isJson(res)) {
          ...
          if (jRes.hasOwnProperty('server_error')) {
            ...
            if (jdsChannel) {
              ...
              jdsChannel.disconnect();
              delete jdsChannel.io.nsps[jdsChannel.nsp]; // jdsChannel.nsp = '/py_...'
              delete jdsChannel;
            }

            // jdsChannel = io('ws://localhost:8000/py_'+GROUP_CHANNEL, {
            jdsChannel = io('ws://ws-jds-eu.herokuapp.com/py_'+GROUP_CHANNEL, {
                auth: {
                  token: 'qPyFMKAdjtfL3Gq5pk2xDgy0SKMpEmLz',
                  uid: jRes.UID,
                  gc: GROUP_CHANNEL
                }
              }
            ...
            jdsChannel.on('connect', () => {
              console.log("connected "+jdsChannel.nsp+" socket");
              ...
              for (var i = 0; i < member.length; i++) {
                let parDiv = document.createElement('DIV');
                    ...
                    let childDiv2 = document.createElement('A');
                        ...
                        childDiv2.setAttribute('data-uid', GROUP_CHANNEL+"/"+member[i].uid);
                        childDiv2.innerText = '---';
                    parDiv.append(childDiv2);
                document.querySelector('._bs2dcpr_container').append(parDiv);
                // GET MEMBER ONLINE STATUS
                jdsChannel.emit('msg', {action: 'user_status', gc: GROUP_CHANNEL, uid: member[i].uid});
                console.log("SENT FROM: "+jdsChannel.nsp);
              }
            });
            ...
            jdsChannel.on('msg', (data) => {
              ...
              //       This socket is for download sockets and member activity, to recieve download progress and member status
              ...
              if (data.hasOwnProperty('response')) {
                ...
                switch (data.response) {
                  case 'user_status':
                    let uData = data.data;
                    console.log("DATA RECVD => "+JSON.stringify(uData));
                    // console.log(uData.gc+"/"+uData.uid);
                    if (!uData.hasOwnProperty('uid')) break; // exit code if data is not of a user
                    const userDiv = document.querySelector('._stat[data-uid="'+uData.gc+"/"+uData.uid+'"]');
                    // console.log(userDiv.getAttribute('data-uid'));
                    if (!userDiv) return false;
                    userDiv.innerHTML = (uData.status == 'connected' && uData.gc == GROUP_CHANNEL) ? "<font color='#4bc64b'>"+uData.status+"</font>" : "<font color='grey'>"+uData.status+"</font>";
                    break;
                  default:
